...
from my_haversine import my_haversine_func, Unit
...
def stations_by_distance(stations, p):
    ...
    for station in stations: # iterating through each item in the stations list
        distance_list.append((station.name,my_haversine_func(station.coord,p))) # haversine function computes the distance between two coordinates on earth   
...
def stations_within_radius(stations, centre, r):
    ...
    for station in stations: 
        d = my_haversine_func(centre, station.coord)
        if d <= r: 
